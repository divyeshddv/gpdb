		...
		if (NULL != pexprEqFilter)
		{
			...
			if (fRangePart)
			{
				filter_dxlnode = CTranslatorExprToDXLUtils::PdxlnRangeFilterEqCmp
								...
								NULL /*mdid_cast_func*/,
                                IMDCast::EmdtNoContext, // castcontext
								ulLevel
								);
			}
			else // list partition
	...
	)
{
	...
	*pfEQComparison = *pfEQComparison || IMDType::EcmptEq == cmp_type;
    ...
    IMDCast::EmdCoerceContext castcontext = IMDCast::EmdtNoContext;
	...
	if (fRangePart) // range partition
	{
		...
		if (!CMDAccessorUtils::FCmpExists(m_pmda, pmdidTypePartKey, pmdidTypeOther, cmp_type)
			&& CMDAccessorUtils::FImplicitCastExists(m_pmda, pmdidTypePartKey, pmdidTypeOther)
		...
		CTranslatorExprToDXLUtils::ExtractCastMdids(pexprNewPartKey->Pop(), &pmdidTypeCastExpr, &mdid_cast_func, &castcontext);
